...
class RotaryEmbedding2D(nn.Module):
    ...
    def __init__(
        ...
        if method == "axial":
            ...
            freqs_x_grid = freqs_x.unsqueeze(0).repeat(
                grid_height, 1, 1
            )  # [H, W, half_dim//2]
            freqs_y_grid = freqs_y.unsqueeze(1).repeat(
                1, grid_width, 1
            )  # [H, W, half_dim//2]
        ...
        # Add extra tokens (e.g., puzzle embeddings) at the beginning with 1D positional encoding
        if num_extra_tokens > 0:
            # Create 1D position encodings for extra tokens
            extra_pos = torch.arange(
                num_extra_tokens, dtype=torch.float32, device=device
            )
            extra_freqs = torch.outer(extra_pos, inv_freq)  # [num_extra_tokens, dim//2]
            extra_emb = torch.cat(
                [extra_freqs, extra_freqs], dim=-1
            )  # [num_extra_tokens, dim]

            # Concatenate extra tokens before spatial tokens
            emb = torch.cat([extra_emb, emb], dim=0)  # [num_extra_tokens + H*W, dim]
