...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
        ...
        if self.config.pos_encodings == "rope":
            if self.config.use_patches and self.config.rope_2d:
                ...
                self.rotary_emb = RotaryEmbedding2D(
                    ...
                    num_extra_tokens=self.puzzle_emb_len,
                )
            else:
                # Use 1D RoPE for standard architecture
                seq_len = (
                    self.num_patches if self.config.use_patches else self.config.seq_len
                )
    ...
    def _input_embeddings(self, input: torch.Tensor, puzzle_identifiers: torch.Tensor):
        ...
        if self.config.use_patches:
            ...
            grid = input.view(
                batch_size, self.config.grid_height, self.config.grid_width
            )
        ...
        if self.config.use_patches:
            ...
            if self.config.upsample_method == "pixel_shuffle":
                ...
                z_H_spatial = z_H_patches.transpose(1, 2).view(
                    batch_size, self.config.hidden_size, patch_grid_h, patch_grid_w
                )
                ...
                output_grid = (
                    patch_predictions.permute(0, 3, 1, 4, 2, 5)
                    .contiguous()
                    .view(
                        batch_size,
                        self.config.vocab_size,
